import { p as push, d as store_get, a6 as copy_payload, a1 as ensure_array_like, b as attr, a2 as stringify, g as getContext, a7 as bind_props, a as pop, a9 as fallback, aa as slot, u as unsubscribe_stores, a8 as assign_payload } from './index-7U-6WjDo.js';
...
import { g as get, w as writable } from './exports-DdSQfVyA.js';
...
function TagInputs($$payload, $$props) {
  ...
  Required($$payload);
  $$payload.out += `<!----> <i class="fa-solid fa-circle-question cursor-help text-gray-500 dark:text-slate-400" title="The tag content"></i> <textarea class="input form-input h-24" maxlength="4096" required>`;
  ...
  if (tag.content) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<p class="text-sm font-medium">Preview</p> <div class="block w-full break-words prose prose-slate dark:prose-invert prose-a:text-blurple rounded-md bg-slate-100 p-3 font-mono text-sm shadow-sm dark:bg-slate-900">${html(marked.parse(tag.content.replace(/\n/g, "\n\n").replace(/{+\s?(user)?name\s?}+/gi, "@" + getContext("user").username)))}</div>`;
  } else {
    $$payload.out += "<!--[!-->";
  }
  $$payload.out += `<!--]--></div>`;
...
//# sourceMappingURL=_page.svelte-BOpVxFZu.js.map
